...
class MyDatabaseHelper extends SQLiteOpenHelper {
    ...
    // ── New: polyunsaturated fat in library ──────────────────────────────────
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        // Users table (now with macro goal columns)
        String CREATE_USER_TABLE = "CREATE TABLE " + TABLE_NAME + " ("
                ...
                + COLUMN_CALORIE_GOAL   + " INTEGER DEFAULT 2000, "
                + COLUMN_PROTEIN_GOAL   + " INTEGER DEFAULT 150, "
                + COLUMN_WATER_GOAL     + " INTEGER DEFAULT 8, "
                + COLUMN_STEP_GOAL      + " INTEGER DEFAULT 10000, "
                + COLUMN_CARB_GOAL      + " INTEGER DEFAULT 250, "
                + COLUMN_FAT_GOAL       + " INTEGER DEFAULT 65, "
                + COLUMN_FIBER_GOAL     + " INTEGER DEFAULT 30, "
                + COLUMN_SUGAR_GOAL     + " INTEGER DEFAULT 50, "
                + COLUMN_SAT_FAT_GOAL   + " INTEGER DEFAULT 20, "
                + COLUMN_POLY_GOAL      + " INTEGER DEFAULT 15, "
                + COLUMN_PROFILE_IMAGE  + " TEXT, "
                + COLUMN_IS_DELETED     + " INTEGER DEFAULT 0)";
        db.execSQL(CREATE_USER_TABLE);

        // Food library (now with poly_fat)
        ...
        // Food log table — now with full macro columns
    ...
    // ────────────────────────────────────────────────────────────────────────
    // onUpgrade — migrate smoothly instead of wiping everything
    // ────────────────────────────────────────────────────────────────────────
    @Override
    ...
    // ────────────────────────────────────────────────────────────────────────
    // Prefill food library
    // ────────────────────────────────────────────────────────────────────────
    private void prefillFoodLibrary(SQLiteDatabase db) {
        // name, cal, carbs, fat, protein, fiber, sugar, satFat, polyFat, size, unit
        // PROTEINS - MEAT
    ...
    private void insertLibraryItem(SQLiteDatabase db, String name, double calories,
        ...
        values.put(COL_LIB_NAME,    name);
        values.put(COL_LIB_CAL,     calories);
        values.put(COL_LIB_CARBS,   carbs);
        values.put(COL_LIB_FAT,     fat);
        values.put(COL_LIB_PRO,     protein);
        values.put(COL_LIB_FIBER,   fiber);
        values.put(COL_LIB_SUGAR,   sugar);
        values.put(COL_LIB_SAT_FAT, satFat);
        values.put(COL_LIB_POLY,    polyFat);
        values.put(COL_LIB_SIZE,    size);
        values.put(COL_LIB_UNIT,    unit);
        db.insert(TABLE_FOOD_LIBRARY, null, values);
    }

    // ────────────────────────────────────────────────────────────────────────
    // Food Log — add entry (now with full macros)
    // ────────────────────────────────────────────────────────────────────────
    public boolean addFoodLogEntry(String email, String mealType, String foodName,
        ...
        cv.put(COL_CARBS,        carbs);
        cv.put(COL_FAT,          fat);
        cv.put(COL_FIBER,        fiber);
        cv.put(COL_SUGAR,        sugar);
        cv.put(COL_SAT_FAT,      satFat);
        cv.put(COL_POLY_FAT,     polyFat);
    ...
    // ────────────────────────────────────────────────────────────────────────
    // Food Log — get entries by meal (returns FoodEntry list)
    // ────────────────────────────────────────────────────────────────────────
    public ArrayList<FoodEntry> getFoodByMeal(String email, String date, String mealType) {
        ...
        String query = "SELECT " + COL_FOOD_ID + ", " + COL_FOOD_NAME + ", "
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                FoodEntry food = new FoodEntry(
                        ...
                        cursor.getDouble(2),
                        cursor.getString(3),
                        (int) cursor.getDouble(4),
                        (int) cursor.getDouble(5)
                );
    ...
    // ────────────────────────────────────────────────────────────────────────
    // MacroTotals — aggregate all macros for a given date (Fragment 1 & 2)
    // ────────────────────────────────────────────────────────────────────────
    ...
    // ────────────────────────────────────────────────────────────────────────
    // Calorie totals per meal type (Fragment 3 pie chart)
    // returns index: 0=Breakfast, 1=Lunch, 2=Dinner, 3=Snacks
    // ────────────────────────────────────────────────────────────────────────
    public double[] getCaloriesPerMeal(String email, String date) {
        double[] result = new double[4];
        // Guard: if either argument is null, return zeros immediately
    ...
    // ────────────────────────────────────────────────────────────────────────
    // Top foods by carbs for a given date (Fragment 1 bottom list)
    // ────────────────────────────────────────────────────────────────────────
    public ArrayList<String[]> getTopFoodsByCarbs(String email, String date, int limit) {
        ...
        Cursor cursor = db.rawQuery(
                "SELECT " + COL_FOOD_NAME + ", " + COL_CARBS
                        + " FROM " + TABLE_FOOD_LOG
                        + " WHERE " + COLUMN_EMAIL + " = ? AND " + COL_LOG_DATE + " = ?"
    ...
    public MacroTotals getMacroGoals(String email) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            goals.carbs    = cursor.getDouble(0);
            goals.fat      = cursor.getDouble(1);
            goals.protein  = cursor.getDouble(2);
            goals.fiber    = cursor.getDouble(3);
            goals.sugar    = cursor.getDouble(4);
            goals.satFat   = cursor.getDouble(5);
            goals.polyFat  = cursor.getDouble(6);
            goals.calories = cursor.getDouble(7);
            cursor.close();
        }
        db.close();
        return goals;
    ...
    public boolean updateMacroGoals(String email, int carbs, int fat, int fiber,
        ...
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_CARB_GOAL,    carbs);
        cv.put(COLUMN_FAT_GOAL,     fat);
        cv.put(COLUMN_FIBER_GOAL,   fiber);
        cv.put(COLUMN_SUGAR_GOAL,   sugar);
        cv.put(COLUMN_SAT_FAT_GOAL, satFat);
        cv.put(COLUMN_POLY_GOAL,    poly);
    ...
    // ────────────────────────────────────────────────────────────────────────
    // Delete food entry (subtracts all macro columns from daily_logs)
    // ────────────────────────────────────────────────────────────────────────
    ...
    public ArrayList<FoodEntry> searchFoodLibrary(String queryText) {
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                // 0=lib_id,  1=food_name,       2=calories,  3=carbs,
                // 4=fat,     5=protein,          6=fiber,     7=sugar,
                // 8=sat_fat, 9=poly_fat,         10=base_serving_size, 11=base_unit
                results.add(new FoodEntry(
                        cursor.getString(1),        // food_name
                        cursor.getDouble(10),       // base_serving_size
                        cursor.getString(11),       // base_unit
                        cursor.getInt(2),           // calories
                        (int) cursor.getDouble(5),  // protein
                        cursor.getDouble(3),        // carbs
                        cursor.getDouble(4),        // fat
                        cursor.getDouble(6),        // fiber
                        cursor.getDouble(7),        // sugar
                        cursor.getDouble(8),        // sat_fat
                        cursor.getDouble(9)         // poly_fat
                ));
            }
            cursor.close();
        }
    ...
    public ArrayList<Integer> getDailyStats(String email, String date) {
        ...
        if (cursor.moveToFirst()) {
            ...
            stats.add(cursor.getInt(1));
            stats.add(cursor.getInt(2));
            stats.add(cursor.getInt(3));
        } else {
    ...
    public boolean doesDailyDataSatisfyHabit(String email, String date, String habitName) {
        ...
        switch (habitName) {
            ...
            case "Get more exercise":
                ArrayList<String> w = getWorkoutsForDate(email, date);
